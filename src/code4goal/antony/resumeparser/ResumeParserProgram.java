...
*/
	...
	public String Transducer(File file) {
		/*if (args.length == 0) {
			System.err
		...
		String outputFileName = (args.length == 2) ? args[1]
				: "parsed_resume.json";*/
		String inputFileName = "E:/AntonyDeepakThomas.pdf";
		
		String outputFileName = file.getName();
		ResumeParserProgram rpp = new ResumeParserProgram();
		try {
			File tikkaConvertedFile = rpp.parseToHTMLUsingApacheTikka(inputFileName);
			if (tikkaConvertedFile != null) {
				JSONObject parsedJSON = loadGateAndAnnie(tikkaConvertedFile);
				JSONObject resultJSON = rpp.resultJSON(parsedJSON);
	...
	public JSONObject resultJSON(JSONObject obj)
	{
		ResumeParserProgram rpp = new ResumeParserProgram();
		  ...
		  resultJSON.put("basics",rpp.basicJSON(obj));
		  //Out.prln(resultJSON);
		  resultJSON.put("work",rpp.workJSON(obj));
		  JSONArray array = rpp.interestsJSON(obj);
		  if(!array.isEmpty()){
		  resultJSON.put("interests",array);
		  }
		  resultJSON.put("education",rpp.educationJSON(obj));
		  resultJSON.put("skills",rpp.skillsJSON(obj));
		  return resultJSON;
	}
